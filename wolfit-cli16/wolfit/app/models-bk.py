...
import json
import os
#to get environment variable from setting files
...
#        return cls.query.order_by(ActivityLog.id.desc()).first()
    ...
    @classmethod
    def log_event(cls, user_id, user_name, details):
     url = os.getenv("LOG_URL") #environment variable is set in the "rundev.sh" file
     new_activity = {
      "user_id": user_id,
      "username": user_name,
     ...
     try:
      r = requests.post(url, json=new_activity)
