...
def test_no_count_post_equals_zero(client, test_user):
	p = Post(title="testing post", body="testing123", vote_count=None)
	p.down_vote(test_user)
...
def test_comment_have_readable(client, test_user, single_post_with_comment):
	c = single_post_with_comment.comments(0)
	assert c.body in str(c)
