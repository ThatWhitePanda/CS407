...
def test_creae_post_not_logged_in(client):
	response = client.get(url_for("create_post"))
	assert response.status_code == 302
	assert "/login" in response.headers["Location"]

def test_upvote(client, test_user):
	response = login(client, test_user, PASSWORD)
	assert response.status_code == 200


def test_create_post_not_registed(client):
	#go to creat_post page
	response = client.get(url_for("create_post"))

	#redirect user to the register page, if not logged in
	assert response.status_code == 302
	assert "/register" in response.headers["location"]

def test_register_authenticated_index(client, test_user):
	#if user is logged in and redirect to index if on register page
...
def test_post_not_logged_in(client, single_post):
	#if the user is trying to post when not logged in, redirect to login
	response = client.post(url_for("post", id=single_post.id))
	assert "/login" in response.headers["location"]

#165
def test_post_comments(client, test_user, single_post):
	login(client, test_user.username, PASSWORD)
	client.single_post.id.addcomment("tst", test_user)
	response = client.post(url_for("post", id=single_post.id))
	assert response.status_code == 200

def  test_login_to_vote(client):
	response = client.get(url_for("up_vote", id=0))
	assert response.status_code == 302
	assert "/login" in response.headers["Location"]

	response = client.get(url_for("down_vote",id=0))
	assert response.status_code == 302
	assert "/login" in response.headers["Location"]
	
	response = client.get(url_for("up_vote_comment", id=0))
	assert response.status_code == 302
	assert "/login" in response.headers["Location"]
	
	response = client.get(url_for("down_vote_comment", id=0))
	assert response.status_code == 302
	assert "/login" in response.headers["Location"]

def test_return_to_post_after_vote(client, test_user, single_post):
	login(client, test_user.username, PASSWORD)
	start_page = url_for("post", id=single_post.id)
	response = client.get(url_for("up_vote", id=single_post.id, next=start_page))
	assert response.status_code == 302
	assert start_page in response.headers["location"]


def test_post_change_Down_count(client, test_user, single_post):
	login(client, test_user.username, PASSWORD)
	start_count = single_post.vote_count
	client.get(url_for("down_vote", id=single_post.id))
	post = db.session.query(Post).filter_by(id=single_post.id).one()
	assert post.vote_count == start_count - 1

def test_post_change_Up_count(client, test_user, single_post):
	login(client, test_user.username, PASSWORD)
	start_count = single_post.vote_count
	client.get(url_for("Up_vote", id=single_post.id))
	post = db.session.query(Post).filter_by(id=single_post.id).one()
	assert post.vote_count == start_count + 1

def test_comment_upvote_count(client, test_user, single_post_with_comment):
	login(client, test_user.username, PASSWORD)
	comment = single_post_with_comment.comments[0]
	start_count = comment.vote_count
	client.get(url_for("up_vote_comment",id=comment.id))
	comment = db.session.query(Comment).filter_by(id=comment.id).one()
	assert comment.vote_count == start_count + 1

def test_comment_downvote_count(client, test_user, single_post_with_comment):
	login(client, test_user.username, PASSWORD)
	comment = single_post_with_comment.comments[0]
	start_count = comment.vote_count
	client.get(url_for("down_vote_comment",id=comment.id))
	comment = db.session.query(Comment).filter_by(id=comment.id).one()
	assert comment.vote_count == start_count - 1
...
def test_register(client):
	...
	assert client.get(url_for("index"), follow_redirects=True)
